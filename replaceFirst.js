...
function buildCheckFunction_opt(pattern) {
    return function (text, index) {
        const start = text.lastIndexOf('<', index);
        if (start === -1) return false;
        const end = text.indexOf('>', index);
...
const isInTagA_opt = buildCheckFunction_opt(/^<a((\s[^<>]*)>|>)[^<>]*<\/a\s*>$/i);
const isInTagAttribute_opt = buildCheckFunction_opt(/^<[^<>]+>$/);
...
function run(text) {
    ...
    mytest(text, replaceFirst);
}

//run('票0房票-房html<a>票房</a>票1房AA\x01BB<a>wng票房e1kk</a> 票 房票3房loong票5房票`房 <a href="票房" alter="nXnwgX9">X票房票房X</a>end票q房wng213<p e>票x房</p><ax票房>票.房</ax><a >票房</a ><img src="票房"/><a >票房</a>票房票房')
//run('<a href="">票房</a><b alter="票房">票.房</b><a >票房</a ><a>票房</a><p class="票房"><p class="票房"><div class="票房">票.房</div></p><p><p class="票房"><p class="票房"><div class="票房">票房</div></p><p>');
// mytest('<a>票房</a>票房123456789012345678901234567890票房', replaceFirst);
